...
namespace SeleniumBase.Framework.Core.Helpers
{
    public class AzureDevOpsHelper
    {
        ...
        public static int GetActiveTestPlan()
        {
            string ApiEndPoint = "_apis/test/plans" + apicallversion;
            JObject jBody = new JObject();
            IRestResponse responseplan = APIServices.Get(DevOpsUrl,ApiEndPoint,jBody);
            List<char> planList = responseplan.Content.ToList();
            
            int activePlan = planList.FindLast(item => item.Equals("Active"));
            Debug.WriteLine(activePlan);
            return activePlan;
        }
